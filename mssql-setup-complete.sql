...
IF EXISTS (SELECT * FROM sys.databases WHERE name = 'QuizPlatformDB')
BEGIN
    ALTER DATABASE QuizPlatformDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE QuizPlatformDB;
END
GO
...
PRINT 'Server=localhost;Database=QuizPlatformDB;Integrated Security=true;TrustServerCertificate=true;';
PRINT '';
